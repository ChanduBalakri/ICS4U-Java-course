...
/* Pseudo Code
 ...
 if user chooses 7 ask them for what field they would like to sort, then sort accordingly from high-low or low-high per request
 if user chooses 8 ask then for what field they would like to search, go through this field's array and check if value is found, if not then print accordingly
 end
 */
...
/* IPO Charts
 ...
 ------------------------------------------------------------------------------------------------------------------------------------------
 Input: choose option 7
 Process: check what option 7 is (Sort)
 ...
 Process: check what option 8 is (Search)
...
public class NewContact {
	...
	public static void main(String[] args) throws IOException {
		while (choice != 9) { // common while loop to only stop when user selects quit option
	...
	private static void makeChoice(int x) throws IOException { // calls methods for certain choices
		...
		else if (x == 7) { // sort entire stored book
			sortBook();
		}
		else if (x == 8) { // search through entire stored book
			searchBook();
		}
	...
	private static void sortBook() { // call this method when sorting book
        String temp = "";  
        System.out.println("What field value would you like to order? (1-name, 2-number 3-dob 4-address 5-email)");
		character = readInt();
        System.out.println("What order would you like the list, A-Z (1) or Z-A (2)");
		order = readInt();
        while (swapped){
            swapped = false;
            if (character == 1) { // if chooses name
            	organize(name, temp);
            }
			if (character == 2) { // if chooses number
				numbOrganize(number, temp);
            }
			if (character == 3) { // if chooses dob
				organize(dob, temp);
			}
			if (character == 4) { // if chooses address
				organize(address, temp);
			}
			if (character == 5) { // if chooses email
				organize(email, temp);
			}
        }
        swapped = true;
	...
	private static void searching (String[]value) {
		for (int i = 0; i < counter; i++) {
			if (flag.compareTo(value[i]) == 0) { 
				found = true;
				key = i;
			}
		}
		if (found == true) { 
			System.out.println(book[key] + " was found");
		}
		else {
	...
	private static void searchBook() throws IOException { // search through book
		System.out.println("What field value would you like to search? (1-name, 2-number 3-dob 4-address 5-email)");
		character = readInt();
